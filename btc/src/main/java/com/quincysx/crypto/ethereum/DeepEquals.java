...
public class DeepEquals
{
    ...
    private static class DualKey
    {
        ...
        {
            if (!(other instanceof DeepEquals.DualKey))
            {
                return false;
            }

            DeepEquals.DualKey that = (DeepEquals.DualKey) other;
    ...
    {
        Set<DeepEquals.DualKey> visited = new HashSet<DeepEquals.DualKey>();
        LinkedList<DeepEquals.DualKey> stack = new LinkedList<DeepEquals.DualKey>();
        stack.addFirst(new DeepEquals.DualKey(a, b));

        while (!stack.isEmpty())
        {
            DeepEquals.DualKey dualKey = stack.removeFirst();
            visited.add(dualKey);
            ...
            for (Field field : fields)
            {
                try
                {
                    DeepEquals.DualKey dk = new DeepEquals.DualKey(field.get(dualKey._key1), field.get(dualKey._key2));
    ...
    private static boolean compareArrays(Object array1, Object array2, LinkedList stack, Set visited)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            DeepEquals.DualKey dk = new DeepEquals.DualKey(Array.get(array1, i), Array.get(array2, i));
            if (!visited.contains(dk))
    ...
    private static boolean compareOrderedCollection(Collection col1, Collection col2, LinkedList stack, Set visited)
    {
        ...
        while (i1.hasNext())
        {
            DeepEquals.DualKey dk = new DeepEquals.DualKey(i1.next(), i2.next());
            if (!visited.contains(dk))
            {   // push contents for further comparison
                stack.addFirst(dk);
            }
    ...
    private static boolean compareUnorderedCollection(Collection col1, Collection col2, LinkedList stack, Set visited)
    {
        ...
        for (Object o : col1)
        {
            ...
            DeepEquals.DualKey dk = new DeepEquals.DualKey(o, other);
            if (!visited.contains(dk))
    ...
    private static boolean compareSortedMap(SortedMap map1, SortedMap map2, LinkedList stack, Set visited)
    {
        ...
        while (i1.hasNext())
        {
            ...
            DeepEquals.DualKey dk = new DeepEquals.DualKey(entry1.getKey(), entry2.getKey());
            if (!visited.contains(dk))
            {   // Push Keys for further comparison
                stack.addFirst(dk);
            }

            dk = new DeepEquals.DualKey(entry1.getValue(), entry2.getValue());
            if (!visited.contains(dk))
    ...
    private static boolean compareUnorderedMap(Map map1, Map map2, LinkedList stack, Set visited)
    {
        ...
        for (Map.Entry entry : (Set<Map.Entry>)map1.entrySet())
        {
            ...
            DeepEquals.DualKey dk = new DeepEquals.DualKey(entry.getKey(), other.getKey());
            if (!visited.contains(dk))
            {   // Push keys for further comparison
                stack.addFirst(dk);
            }

            dk = new DeepEquals.DualKey(entry.getValue(), other.getValue());
            if (!visited.contains(dk))
            {   // Push values for further comparison
                stack.addFirst(dk);
            }
